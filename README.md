...
OpenSync 是为飞牛 fnOS / 飞牛 NAS 准备的 AList / OpenList 自动同步工具。它把本地目录、网盘、对象存储、WebDAV 等存储端统一接到 AList / OpenList 后，再通过可视化任务完成自动备份、归档和迁移。

如果你在飞牛 NAS 上想找一个类似群晖 Cloud Sync 的平替方案，用来把照片库、影音库、下载目录或文档目录同步到网盘、对象存储或另一台存储设备，OpenSync 就是面向这个场景做的。它支持手动、间隔和 Cron 定时执行，能看到实时进度、失败原因和历史记录，也能在任务完成后推送通知。

它也可以部署在其他 NAS、家庭服务器或普通 Docker 环境中，但核心使用体验优先围绕飞牛 NAS 下的自动同步来设计。
...
- 在飞牛 fnOS / 飞牛 NAS 上，把本地目录定时备份到 AList / OpenList 支持的远端存储。
- 在飞牛 NAS 上替代群晖 Cloud Sync，完成跨网盘、跨对象存储、跨 WebDAV 的自动同步。
